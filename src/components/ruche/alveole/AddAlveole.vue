<template>
  <q-card style="width: 700px; border-radius: 8px;">
    <q-card-section class="row items-center q-pb-none q-pt-none">
      <div class="text-h6 q-mt-md q-ml-md">Ajout d'alvéole dans la Box</div>
    </q-card-section>
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const $q = useQuasar()
    let formData = ref({})
    let addAlveoleMode = inject("addAlveoleMode")
    ...
    let onSubmit = async () => {
      try {
        formData.value.id = props.infoRuche.id
        ...
        alveoleAdding.value = true
        addAlveoleMode.value = false
      })
      } catch (error) {
        submitting.value = false
        console.log(error);
